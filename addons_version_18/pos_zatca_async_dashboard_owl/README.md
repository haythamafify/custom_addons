# pos_zatca_async_v18 — ZATCA EDI Async Fix for POS (Odoo 18)
...
cp -r pos_zatca_async_v18 /path/to/your/addons/
...
## Dashboard (ZATCA Sync Monitor)

- المكان: Point of Sale → Reporting → ZATCA Sync Monitor
- Stat Cards بتعرض أرقام اليوم (Pending / Sent / Error)
- قائمة متابعة للفواتير بحالة `to_send` و `error`
- زر Force Run لتشغيل الـ Cron يدويًا
...
| | v17 | v18 |
|--|-----|-----|
| EDI tracking | `account.edi.document` | مباشرة على `account.move` |
| dependency | `account_edi` | محذوف (merged في `account`) |
| EDI state field | `edi_document.state` | `move.l10n_sa_edi_state` |
| Cron queries | `account.edi.document` | `account.move` مباشرة |
...
│   └── zatca_sync_monitor_data.xml ← default dashboard record
├── views/
│   └── zatca_sync_monitor_views.xml
├── security/
│   └── ir.model.access.csv
└── static/
    └── description/
